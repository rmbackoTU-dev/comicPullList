# TODO FILE 

**Reminder keep UI elements for JSP and forms simple** 

[ X ] Move a back up of current comic book features to the complex branch
[ X ] Update TODO.md and README.md with new development plan
[ X ] Create a feature branch for pruning, and CRUD feature development
[ ] Prune the automation / Non-CRUD features from Java application
[ ] Create a basic Servlet for Create Comic issue
[ ] Create a MySQL Database that contains information neccesary to build a Issue object
[ ]Create a JBDC to connect to the database.
[ ] Write newly created comics to the database using IN-LINE SQL
[ ] Read comics from the database using IN-LINE SQL
[ ] Delete comics in the database using IN-LINE SQL
[ ] Create a JSP Form that allows comic creation
[ ] Create a JSP Page that allows comics to be displayed
[ ] Add a feature to the JSP display page that allows comics to be deleted
[ ] Add a way to navigate to the JSP Create comic page with the intent to update a comic tie the intent to a comic update

